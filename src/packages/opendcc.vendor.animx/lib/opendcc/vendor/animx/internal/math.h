...
namespace adsk
{
    ...
    constexpr long kTicksPerSecondInTicks = 141120000;
#else
    constexpr long kTicksPerSecondInTicks = 6000;
#endif
    ...
    {
        using Ticks = long;
        Ticks ticks = (Ticks)llround(std::max(std::min((double)std::numeric_limits<Ticks>::max(), (double)(time * kTicksPerSecondInTicks)),
