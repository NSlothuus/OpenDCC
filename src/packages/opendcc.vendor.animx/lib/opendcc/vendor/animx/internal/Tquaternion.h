...
namespace adsk
{
    ...
    struct Tquaternion
    {
        ...
        inline Tquaternion(Quaternion q)
            : x(q.x)
            , y(q.y)
            , z(q.z)
            , w(q.w)
        ...
        inline operator Quaternion() { return Quaternion { x, y, z, w }; }
        ...
        inline Tquaternion scale(double s) const { return Tquaternion(s * x, s * y, s * z, s * w); }
        ...
        inline Tquaternion operator-(const Tquaternion &r) const { return Tquaternion(x - r.x, y - r.y, z - r.z, w - r.w); }
        ...
        inline static double computeW(double x, double y, double z) { return sqrt(1.0 - x * x - y * y - z * z); }
        ...
        Tquaternion pow(double scalar) const { return (scalar * log()).exp(); }
    };
    ...
    Tquaternion bezierPt(const Tquaternion &qPrev, const Tquaternion &q, const Tquaternion &qNext, const bool findNextCtrlPt)
    {
    ...
    Tquaternion bezier(const Tquaternion &p, const Tquaternion &a, const Tquaternion &b, const Tquaternion &q, const double t)
    {
        Tquaternion tmp = slerp(a, b, t);
        return slerp(slerp(slerp(p, a, t), tmp, t), slerp(tmp, slerp(b, q, t), t), t);
    }

    inline Tquaternion operator*(double s, const Tquaternion &rightQ)
    {
        Tquaternion qScale(rightQ);
        return qScale.scaleIt(s);
    }
