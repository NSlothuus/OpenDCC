...
namespace adsk
{
    ...
    */
    /// using of values like in common animation program
    enum class InfinityType
    {
        Constant = 0, //!< Constant
        Linear = 1, //!< Linear
        Cycle = 3, //!< Cycle
        CycleRelative = 4, //!< CycleRelative
    ...
    struct Tangent
    {
        ...
        seconds x = 1; //!< Tangent x value
        seconds y = 0; //!< Tangent y value
    };
    ...
    public:
        ...
        explicit KeyId(uint64_t value)
            : m_value(value)
        {
        }
        bool operator<(const KeyId &other) const { return m_value < other.m_value; }
        bool operator>(const KeyId &other) const { return m_value > other.m_value; }
        bool operator==(const KeyId &other) const { return m_value == other.m_value; }
    ...
    struct Keyframe : public KeyTimeValue
    {
        ...
        bool linearInterpolation =
            false; //!< Should curve be linearly interpolated? True if tangents of this key and the neighboring one are both linear.
        KeyId id;
    ...
    */
    DLL_EXPORT double evaluateCurve(double time, const ICurve &curve);
    ...
    */
    DLL_EXPORT double evaluateCurveSegment(SpanInterpolationMethod interpolationMethod, CurveInterpolatorMethod curveInterpolatorMethod, double time,
    ...
    */
    DLL_EXPORT bool evaluateQuaternionInfinity(double &time, double firstTime, Quaternion firstValue, double lastTime, Quaternion lastValue,
    ...
    */
    DLL_EXPORT double evaluateInfinity(double time, const ICurve &curve, Infinity infinity);
    ...
    */
    DLL_EXPORT Quaternion evaluateQuaternion(seconds time, CurveRotationInterpolationMethod interpolationMethod,
                                             SpanInterpolationMethod spanInterpolationMethod, seconds startTime, Quaternion startValue,
                                             seconds endTime, Quaternion endValue, TangentType tangentType, Quaternion prevValue,
    ...
    */
    DLL_EXPORT Quaternion evaluateQuaternionCurve(double time, const ICurve &pcX, const ICurve &pcY, const ICurve &pcZ,
    ...
    */
    DLL_EXPORT void autoTangent(bool calculateInTangent, KeyTimeValue key, KeyTimeValue *prevKey, KeyTimeValue *nextKey,
