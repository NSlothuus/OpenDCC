...
namespace adsk
{
    ...
    double tangentAngle(const ICurve &curve, unsigned int index, bool inTangent, double xScale = 1.0, double yScale = 1.0)
    {
        Keyframe key;
        ...
        double angle(equivalent(tanValue, 0.0) ? 0.0 : sign(tanValue) * 90.0);
    ...
    double evaluateCurve(double time, const ICurve &curve)
    {
        ...
        return evaluateCurveSegment(prev.spanInterpolationMethod(), prev.curveInterpolationMethod(curve.isWeighted()), time, prev.time, prev.value,
                                    prev.time + prev.tanOut.x * oneThird, prev.value + prev.tanOut.y * oneThird, next.time - next.tanIn.x * oneThird,
                                    next.value - next.tanIn.y * oneThird, next.time, next.value);
    }

    //! Evaluate an individual curve segment
    double evaluateCurveSegment(SpanInterpolationMethod interpolationMethod, CurveInterpolatorMethod curveInterpolatorMethod, double time,
                                double startX, double startY, double x1, double y1, double x2, double y2, double endX, double endY)
    {
        double result = 0.0;
    ...
    bool evaluateQuaternionInfinity(double &time, double firstTime, Quaternion firstValue, double lastTime, Quaternion lastValue,
                                    InfinityType preInfinityType, InfinityType postInfinityType, Quaternion &qOffset, Quaternion &qStart,
                                    bool &inverse)
    {
    ...
    Tquaternion findClosestKeyframe(double time, const ICurve &pcX, const ICurve &pcY, const ICurve &pcZ, bool forward, bool atStart,
                                    Keyframe *keyframe)
    {
        ...
        if (iX >= 0 && iX < nX)
        {
            ...
            tX = kx.time;
            if ((iX < nY && equivalent(tX, ky.time)) && (iX < nZ && equivalent(tX, kz.time)))
            {
                if (keyframe)
                    *keyframe = kz;

                return Tquaternion(kx.value, ky.value, kz.value, (float)kz.quaternionW /* deliberate cast to float */);
            }
        }
        else if (iX < 0)
        {
    ...
    Quaternion evaluateQuaternionCurve(double time, const ICurve &pcX, const ICurve &pcY, const ICurve &pcZ,
                                       CurveRotationInterpolationMethod interpolationMethod)
    {
        ...
        const bool needsInfPostProcessing = evaluateQuaternionInfinity(time, first.time, firstQ, last.time, lastQ, pcZ.preInfinityType(),
                                                                       pcZ.postInfinityType(), qInfOffsetQ, qInfStartQ, isInfInversed);
        ...
        Quaternion _q = evaluateQuaternion((seconds)time, interpolationMethod, start.spanInterpolationMethod(), (seconds)start.time, q1,
                                           (seconds)end.time, q2, start.tanOut.type, q0, q3);
    ...
    Quaternion evaluateQuaternion(seconds time, CurveRotationInterpolationMethod interpolationMethod, SpanInterpolationMethod spanInterpolationMethod,
                                  seconds startTime, Quaternion startValue, seconds endTime, Quaternion endValue, TangentType tangentType,
                                  Quaternion prevValue, Quaternion nextValue)
    {
        Tquaternion q1(startValue);
        Tquaternion q2(endValue);
        Tquaternion q;
        bool notEvaluated = false;
        double param = equivalent(startTime, endTime) ? 1.0 : (time - startTime) / (double)(endTime - startTime);
    ...
    double evaluateInfinity(double time, const ICurve &curve, Infinity infinity)
    {
        Keyframe first, last;
        if (!curve.first(first) || !curve.last(last))
            return 0.0;
        ...
        if (infinity == Infinity::Pre)
        {
            ...
            else if (preInfinityType == InfinityType::Cycle || preInfinityType == InfinityType::CycleRelative)
            {
                factoredTime = last.time - factoredTime;
            }
        ...
        else
        {
            ...
            else if (postInfinityType == InfinityType::Cycle || postInfinityType == InfinityType::CycleRelative)
            {
                factoredTime = first.time + factoredTime;
            }
        ...
        /// we do not use toTickDoubleTime because it may lead to crush
        /// however, in our tests, using/unusing of TickDoubleTime does not change anything
        // value = evaluateCurve(toTickDoubleTime(factoredTime), curve);
        value = evaluateCurve(factoredTime, curve);
    ...
    void autoTangent(bool calculateInTangent, KeyTimeValue key, KeyTimeValue *prevKey, KeyTimeValue *nextKey,
                     CurveInterpolatorMethod curveInterpolationMethod, seconds &tanX, seconds &tanY)
    {
