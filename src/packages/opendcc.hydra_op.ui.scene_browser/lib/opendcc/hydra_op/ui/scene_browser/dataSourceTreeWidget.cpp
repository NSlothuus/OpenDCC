...
namespace
{
    ...
    {
    public:
        Hdui_DataSourceTreeWidgetItem(const HdDataSourceLocator &locator, QTreeWidgetItem *parentItem, HdDataSourceBaseHandle dataSource)
        ...
        void SetDirty(const HdDataSourceBaseHandle &dataSource)
        {
            if (_childrenBuilt)
            {
                if (HdContainerDataSourceHandle containerDataSource = HdContainerDataSource::Cast(dataSource))
                {
                    ...
                    {
                        if (Hdui_DataSourceTreeWidgetItem *childItem = dynamic_cast<Hdui_DataSourceTreeWidgetItem *>(child(i)))
                        {
                ...
                else if (HdVectorDataSourceHandle vectorDataSource = HdVectorDataSource::Cast(dataSource))
                {
                    ...
                    HdVectorDataSourceHandle existingVectorDataSource = HdVectorDataSource::Cast(_dataSource);

                    // confirm that existing data source is also a vector
                    // of the same length (could reuse items but probably not
                    // worth the extra complexity)
                    if (!existingVectorDataSource || childCount() != static_cast<int>(vectorDataSource->GetNumElements()))
                    {
                        _dataSource = dataSource;
                        _RebuildChildren();
                        return;
        ...
        HdDataSourceBaseHandle GetDataSource() { return _dataSource; }

        HdDataSourceLocator GetLocator() { return _locator; }
        ...
        void _BuildChildren()
        {
            ...
            else if (HdVectorDataSourceHandle vectorDs = HdVectorDataSource::Cast(_dataSource))
            {
                ...
                for (size_t i = 0, e = vectorDs->GetNumElements(); i != e; ++i)
                {
                    ...
                    new Hdui_DataSourceTreeWidgetItem(_locator.Append(TfToken(buffer)), this, vectorDs->GetElement(i));
                }
            }
        }
    };
    ...
    : QTreeWidget(parent)
{
    ...
    connect(this, &QTreeWidget::itemExpanded, [](QTreeWidgetItem *item) {
        if (Hdui_DataSourceTreeWidgetItem *dsItem = dynamic_cast<Hdui_DataSourceTreeWidgetItem *>(item))
        {
            dsItem->WasExpanded();
        }
    });
    ...
    connect(this, &QTreeWidget::itemSelectionChanged, [this]() {
        ...
        if (Hdui_DataSourceTreeWidgetItem *dsItem = dynamic_cast<Hdui_DataSourceTreeWidgetItem *>(items[0]))
        {
...
void HduiDataSourceTreeWidget::SetPrimDataSource(const SdfPath &primPath, HdContainerDataSourceHandle const &dataSource)
{
    clear();
    if (dataSource)
    {
        Hdui_DataSourceTreeWidgetItem *item = new Hdui_DataSourceTreeWidgetItem(HdDataSourceLocator(), invisibleRootItem(), dataSource);

        item->setText(0, primPath.GetName().c_str());
    }
}

void HduiDataSourceTreeWidget::PrimDirtied(const SdfPath &primPath, const HdContainerDataSourceHandle &primDataSource,
...
void HduiDataSourceTreeWidget::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    QMenu menu;
    QAction *dumpToStdout = menu.addAction("Dump to stdout", [this]() { _DumpDataSource(this, std::cout); });
