...
public:
    ...
    virtual void SetRegisteredSceneIndex(const std::string &registeredName, HdSceneIndexBaseRefPtr sceneIndex);

    // Sets which scene index we are inspecting.
    void SetSceneIndex(const std::string &displayName, HdSceneIndexBaseRefPtr sceneIndex, bool pullRoot);
