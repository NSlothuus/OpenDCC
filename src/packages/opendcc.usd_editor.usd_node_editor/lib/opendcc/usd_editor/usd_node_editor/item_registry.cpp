...
UsdItemRegistry::UsdItemRegistry(UsdEditorGraphModel& model)
    : m_model(model)
{
}

BasicLiveConnectionItem* UsdItemRegistry::make_live_connection(const NodeEditorScene& scene, const NodeEditorView& view, const Port& port)
{
...
ConnectionItem* UsdItemRegistry::make_connection(const NodeEditorScene& scene, const ConnectionId& connection_id)
{
...
NodeItem* UsdItemRegistry::make_node(const NodeEditorScene& scene, const NodeId& node_id)
{
    ...
    if (UsdUIBackdrop(prim))
        return new BackdropNodeItem(m_model, node_id, prim.GetName().GetString());

    return new UsdPrimNodeItem(m_model, node_id, prim.GetName().GetString(), false);
}

QGraphicsItem* UsdItemRegistry::make_live_node(const NodeEditorScene& scene, const std::string& type)
{
    const auto parent_prim = m_model.get_prim_for_node(SdfPath::AbsoluteRootPath().GetString());
    ...
    auto node_item = new UsdLiveNodeItem(m_model, name, TfToken(type), parent_prim.GetPath());
