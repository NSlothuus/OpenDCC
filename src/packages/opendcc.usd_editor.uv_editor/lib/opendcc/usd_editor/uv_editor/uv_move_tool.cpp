...
UvMoveTool::UvMoveTool(UVEditorGLWidget* widget)
    : UvSelectTool(widget)
    , m_manipulator(this)
{
    update_command();
...
bool UvMoveTool::on_mouse_press(QMouseEvent* event) /* override */
{
    switch (current_mode())
    {
    case Mode::Select:
...
bool UvMoveTool::on_mouse_move(QMouseEvent* event) /* override */
{
    switch (current_mode())
    {
    case Mode::Select:
...
bool UvMoveTool::on_mouse_release(QMouseEvent* event) /* override */
{
    switch (current_mode())
    {
    case Mode::Select:
...
void UvMoveTool::draw(ViewportUiDrawManager* draw_manager)
{
    switch (current_mode())
    {
    case Mode::Move:
...
bool UvMoveTool::is_working() const
{
    return UvSelectTool::is_working() || current_mode() != Mode::None;
}

UvMoveTool::Mode UvMoveTool::current_mode() const
{
    ...
    if (select && !move)
    {
        return UvMoveTool::Mode::Select;
    }
    else if (move)
    {
        return UvMoveTool::Mode::Move;
    }
    else
    {
        return UvMoveTool::Mode::None;
    }
}

void UvMoveTool::update_command()
{
    ...
    m_command = CommandRegistry::create_command<UvMoveCommand>("uv_move");
