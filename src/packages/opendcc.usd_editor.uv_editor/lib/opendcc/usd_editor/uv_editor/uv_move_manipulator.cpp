...
#include "opendcc/usd_editor/uv_editor/uv_move_manipulator.h"
...
UvMoveManipulator::UvMoveManipulator(UvTool* tool)
    : m_tool(tool)
{
    create_direction_handles();
}

UvMoveManipulator::~UvMoveManipulator() {}

void UvMoveManipulator::on_mouse_press(QMouseEvent* event)
{
    ...
    m_click = screen_to_world(screen.x(), screen.y(), frustum, w, h);

    m_prev_pos = m_pos;
}

void UvMoveManipulator::on_mouse_move(QMouseEvent* event)
{
    ...
    const auto world = screen_to_world(screen.x(), screen.y(), frustum, w, h);

    m_delta = world - m_click;
    ...
    m_pos = m_prev_pos + m_delta;
}

void UvMoveManipulator::on_mouse_release(QMouseEvent* event)
{
    m_direction = Direction::None;
...
void UvMoveManipulator::draw(ViewportUiDrawManager* manager)
{
    ...
    // x
    {
        ...
        axis_info.length = s_axis_length;
        draw_axis(manager, axis_info, id);

        const auto end = axis_info.begin + axis_info.direction * axis_info.length;
        const auto half_base = s_triangle_base / 2.0f;
        const auto height = s_triangle_height;

        TriangleInfo triangle_info;
        triangle_info.mvp = mvp;
        triangle_info.a = end - GfVec2f(0.0f, half_base);
        triangle_info.b = end + GfVec2f(0.0f, half_base);
        triangle_info.c = end + GfVec2f(height, 0.0f);
        triangle_info.color = color;
        draw_triangle(manager, triangle_info, id);
    }
    ...
    // y
    {
        ...
        const auto axis_end = axis_info.begin + axis_info.direction * axis_info.length;
        const auto half_base = s_triangle_base / 2.0f;
        const auto height = s_triangle_height;

        TriangleInfo triangle_info;
        triangle_info.mvp = mvp;
        triangle_info.a = axis_end - GfVec2f(half_base, 0.0f);
        triangle_info.b = axis_end + GfVec2f(half_base, 0.0f);
        triangle_info.c = axis_end + GfVec2f(0.0f, height);
        triangle_info.color = color;
        draw_triangle(manager, triangle_info, id);
    }
...
bool UvMoveManipulator::move_started() const
{
    return m_direction != Direction::None;
}

const PXR_NS::GfVec2f& UvMoveManipulator::get_delta() const
{
    return m_delta;
}

void UvMoveManipulator::set_pos(const PXR_NS::GfVec2f& pos)
{
    m_pos = pos;
}

void UvMoveManipulator::create_direction_handles()
{
    auto widget = m_tool->get_widget();
    auto manager = widget->get_draw_manager();
...
std::unordered_map<UvMoveManipulator::Direction, PXR_NS::GfVec4f> UvMoveManipulator::get_colors(const uint32_t hover_id) const
{
    std::unordered_map<UvMoveManipulator::Direction, PXR_NS::GfVec4f> result;
...
PXR_NS::GfVec2f UvMoveManipulator::screen_to_clip(const int x, const int y) const
{
...
PXR_NS::GfVec2f UvMoveManipulator::screen_to_clip(PXR_NS::GfVec2f pos) const
{
    const auto widget = m_tool->get_widget();
    const auto w = widget->width();
    const auto h = widget->height();
