...
namespace maddy
{
    ...
    public:
         ...
         */
        BlockParser(std::function<void(std::string&)> parseLineCallback,
    ...
    protected:
        ...
        uint32_t getIndentationWidth(const std::string& line) const
        {
            ...
            uint32_t indentation = static_cast<uint32_t>(std::count_if(line.begin(), line.end(), [&hasMetNonSpace](unsigned char c) {
                if (hasMetNonSpace)
                {
                    return false;
                }
