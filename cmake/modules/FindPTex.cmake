...
# the License and the following modification to it: Section 6 Trademarks. deleted and replaced with:
#
# 1. Trademarks. This License does not grant permission to use the trade names, trademarks, service marks, or product
#   names of the Licensor and its affiliates, except as required for reproducing the content of the NOTICE file.
#
...
if(WIN32)
    find_path(
        PTEX_INCLUDE_DIR
        NAMES Ptexture.h
        HINTS "${PTEX_LOCATION}/include" "$ENV{PTEX_LOCATION}/include"
        PATHS "$ENV{PROGRAMFILES}/Ptex/include" /usr/include
        DOC "The directory where Ptexture.h resides")
    find_library(
        PTEX_LIBRARY
        NAMES Ptex32 Ptex32s Ptex
        HINTS "${PTEX_LOCATION}/lib64" "${PTEX_LOCATION}/lib" "$ENV{PTEX_LOCATION}/lib64" "$ENV{PTEX_LOCATION}/lib"
        PATHS "$ENV{PROGRAMFILES}/Ptex/lib" /usr/lib64 /usr/lib /usr/lib/w32api /usr/local/lib64 /usr/local/lib
...
elseif(APPLE)
    ...
    find_library(
        PTEX_LIBRARY
        NAMES Ptex libPtex.a
        PATHS "${PTEX_LOCATION}/lib" "$ENV{PTEX_LOCATION}/lib"
        DOC "The Ptex Library")
else()
    find_path(
        PTEX_INCLUDE_DIR
        NAMES Ptexture.h
        HINTS "${PTEX_LOCATION}/include" "${PTEX_LOCATION}/include/Ptex" "${PTEX_LOCATION}/include/wdas"
              "$ENV{PTEX_LOCATION}/include" "$ENV{PTEX_LOCATION}/include/Ptex" "$ENV{PTEX_LOCATION}/include/wdas"
