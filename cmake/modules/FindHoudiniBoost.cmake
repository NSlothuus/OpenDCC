...
function(_boost_get_existing_target component target_var)
    ...
    if(component MATCHES "^([a-z_]*)(python|numpy)([1-9])\\.?([0-9])?$")
        ...
        list(APPEND names "${CMAKE_MATCH_1}${CMAKE_MATCH_2}" # python
             "${CMAKE_MATCH_1}${CMAKE_MATCH_2}${CMAKE_MATCH_3}" # pythonX
             "${CMAKE_MATCH_1}${CMAKE_MATCH_2}${CMAKE_MATCH_3}${CMAKE_MATCH_4}" # pythonXY
        )
    endif()
    # https://github.com/boost-cmake/boost-cmake uses boost::file_system etc. So handle similar constructions of target
    # names
    ...
    foreach(prefix Boost boost)
        foreach(name IN LISTS names)
            if(TARGET "${prefix}::${name}")
                # The target may be an INTERFACE library that wraps around a single other target for compatibility.
                # Unwrap this layer so we can extract real info.
...
function(_boost_set_legacy_variables_from_config)
    ...
    find_path(
        Boost_INCLUDE_DIR
        NAMES hboost/version.hpp hboost/config.hpp
        HINTS ${Boost_INCLUDE_DIRS}
        NO_DEFAULT_PATH)
    if(NOT Boost_VERSION_MACRO OR NOT Boost_LIB_VERSION)
        set(version_file ${Boost_INCLUDE_DIR}/hboost/version.hpp)
...
# -------------------------------------------------------------------------------
# Before we go searching, check whether a boost cmake package is available, unless the user specifically asked NOT to
# search for one.
#
...
if(NOT Boost_NO_BOOST_CMAKE)
    ...
    # If we found a boost cmake package, then we're done. Print out what we found. Otherwise let the rest of the module
    # try to find it.
    if(Boost_FOUND)
        # Convert component found variables to standard variables if required Necessary for legacy boost-cmake and 1.70
        # builtin BoostConfig
...
#
# Print debug text if Boost_DEBUG is set. Call example: _Boost_DEBUG_PRINT("${CMAKE_CURRENT_LIST_FILE}"
...
#
# _Boost_DEBUG_PRINT_VAR(file line variable_name [ENVIRONMENT] [SOURCE "short explanation of origin of var value"])
#
...
macro(_Boost_ADJUST_LIB_VARS basename)
    if(Boost_INCLUDE_DIR)
        if(Boost_${basename}_LIBRARY_DEBUG AND Boost_${basename}_LIBRARY_RELEASE)
            # if the generator is multi-config or if CMAKE_BUILD_TYPE is set for single-config generators, set optimized
            # and debug libraries
...
function(_Boost_GUESS_COMPILER_PREFIX _ret)
    ...
    elseif(MINGW)
        if(Boost_VERSION_STRING VERSION_LESS 1.34)
            ...
            _boost_compiler_dumpversion(_boost_COMPILER_VERSION _boost_COMPILER_VERSION_MAJOR
                                        _boost_COMPILER_VERSION_MINOR)
            if(Boost_VERSION_STRING VERSION_GREATER_EQUAL 1.73 AND _boost_COMPILER_VERSION_MAJOR VERSION_GREATER_EQUAL
                                                                   5)
    ...
    elseif(UNIX)
        ...
        if(NOT Boost_VERSION_STRING VERSION_LESS 1.69.0)
            # From GCC 5 and clang 4, versioning changes and minor becomes patch. For those compilers, patch is exclude
            # from compiler tag in Boost 1.69+ library naming.
            ...
            if(Boost_VERSION_STRING VERSION_LESS 1.34)
                ...
                if(APPLE)
                    if(Boost_VERSION_STRING VERSION_LESS 1.36.0)
                        # In Boost <= 1.35.0, there is no mangled compiler name for the macOS/Darwin version of GCC.
                        set(_boost_COMPILER "")
                    else()
                        # In Boost 1.36.0 and newer, the mangled compiler name used on macOS/Darwin is "xgcc".
    ...
    _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "_boost_COMPILER" SOURCE "guessed")
    set(${_ret}
        ${_boost_COMPILER}
        PARENT_SCOPE)
endfunction()
...
function(_Boost_COMPONENT_DEPENDENCIES component _ret)
    ...
    # * Indent s;^set(;    set(;;
    # * Add conditionals s;Scanning /path/to/boost/sources/boost_\(.*\)_\(.*\)_\(.*);  elseif(NOT Boost_VERSION_STRING
    #   VERSION_LESS \1\.\2\.\3 AND Boost_VERSION_STRING VERSION_LESS xxxx);
    #
    # This results in the logic seen below, but will require the xxxx replacing with the following Boost release version
    # (or the next minor version to be released, e.g. 1.59 was the latest at the time of writing, making 1.60 the next.
    # Identical consecutive releases were then merged together by updating the end range of the first block and removing
    # the following redundant blocks.
    #
...
function(_Boost_COMPONENT_HEADERS component _hdrs)
    ...
    # Note: new boost components will require adding here.  The header must be present in all versions of Boost
    # providing a library.
    set(_Boost_ATOMIC_HEADERS "hboost/atomic.hpp")
    set(_Boost_CHRONO_HEADERS "hboost/chrono.hpp")
    set(_Boost_CONTAINER_HEADERS "hboost/container/container_fwd.hpp")
    set(_Boost_CONTRACT_HEADERS "hboost/contract.hpp")
    if(Boost_VERSION_STRING VERSION_LESS 1.61.0)
        set(_Boost_CONTEXT_HEADERS "hboost/context/all.hpp")
    else()
        set(_Boost_CONTEXT_HEADERS "hboost/context/detail/fcontext.hpp")
    endif()
    set(_Boost_COROUTINE_HEADERS "hboost/coroutine/all.hpp")
    set(_Boost_DATE_TIME_HEADERS "hboost/date_time/date.hpp")
    set(_Boost_EXCEPTION_HEADERS "hboost/exception/exception.hpp")
    set(_Boost_FIBER_HEADERS "hboost/fiber/all.hpp")
    set(_Boost_FILESYSTEM_HEADERS "hboost/filesystem/path.hpp")
    set(_Boost_GRAPH_HEADERS "hboost/graph/adjacency_list.hpp")
    set(_Boost_GRAPH_PARALLEL_HEADERS "hboost/graph/adjacency_list.hpp")
    set(_Boost_IOSTREAMS_HEADERS "hboost/iostreams/stream.hpp")
    set(_Boost_LOCALE_HEADERS "hboost/locale.hpp")
    set(_Boost_LOG_HEADERS "hboost/log/core.hpp")
    set(_Boost_LOG_SETUP_HEADERS "hboost/log/detail/setup_config.hpp")
    set(_Boost_MATH_HEADERS "hboost/math_fwd.hpp")
    set(_Boost_MATH_C99_HEADERS "hboost/math/tr1.hpp")
    set(_Boost_MATH_C99F_HEADERS "hboost/math/tr1.hpp")
    set(_Boost_MATH_C99L_HEADERS "hboost/math/tr1.hpp")
    set(_Boost_MATH_TR1_HEADERS "hboost/math/tr1.hpp")
    set(_Boost_MATH_TR1F_HEADERS "hboost/math/tr1.hpp")
    set(_Boost_MATH_TR1L_HEADERS "hboost/math/tr1.hpp")
    set(_Boost_MPI_HEADERS "hboost/mpi.hpp")
    set(_Boost_MPI_PYTHON_HEADERS "hboost/mpi/python/config.hpp")
    set(_Boost_NUMPY_HEADERS "hboost/python/numpy.hpp")
    set(_Boost_NOWIDE_HEADERS "hboost/nowide/cstdlib.hpp")
    set(_Boost_PRG_EXEC_MONITOR_HEADERS "hboost/test/prg_exec_monitor.hpp")
    set(_Boost_PROGRAM_OPTIONS_HEADERS "hboost/program_options.hpp")
    set(_Boost_PYTHON_HEADERS "hboost/python.hpp")
    set(_Boost_RANDOM_HEADERS "hboost/random.hpp")
    set(_Boost_REGEX_HEADERS "hboost/regex.hpp")
    set(_Boost_SERIALIZATION_HEADERS "hboost/serialization/serialization.hpp")
    set(_Boost_SIGNALS_HEADERS "hboost/signals.hpp")
    set(_Boost_STACKTRACE_ADDR2LINE_HEADERS "hboost/stacktrace.hpp")
    set(_Boost_STACKTRACE_BACKTRACE_HEADERS "hboost/stacktrace.hpp")
    set(_Boost_STACKTRACE_BASIC_HEADERS "hboost/stacktrace.hpp")
    set(_Boost_STACKTRACE_NOOP_HEADERS "hboost/stacktrace.hpp")
    set(_Boost_STACKTRACE_WINDBG_CACHED_HEADERS "hboost/stacktrace.hpp")
    set(_Boost_STACKTRACE_WINDBG_HEADERS "hboost/stacktrace.hpp")
    set(_Boost_SYSTEM_HEADERS "hboost/system/config.hpp")
    set(_Boost_TEST_EXEC_MONITOR_HEADERS "hboost/test/test_exec_monitor.hpp")
    set(_Boost_THREAD_HEADERS "hboost/thread.hpp")
    set(_Boost_TIMER_HEADERS "hboost/timer.hpp")
    set(_Boost_TYPE_ERASURE_HEADERS "hboost/type_erasure/config.hpp")
    set(_Boost_UNIT_TEST_FRAMEWORK_HEADERS "hboost/test/framework.hpp")
    set(_Boost_WAVE_HEADERS "hboost/wave.hpp")
    set(_Boost_WSERIALIZATION_HEADERS "hboost/archive/text_wiarchive.hpp")
    if(WIN32)
        set(_Boost_BZIP2_HEADERS "hboost/iostreams/filter/bzip2.hpp")
        set(_Boost_ZLIB_HEADERS "hboost/iostreams/filter/zlib.hpp")
    endif()
...
#
# TODO (Re)Check compiler features on (every?) release ??? One may use the following command to get the files to check:
#
...
if(NOT Boost_INCLUDE_DIR)
    ...
    # Try to find Boost by stepping backwards through the Boost versions we know about. Build a list of path suffixes
    # for each version.
    set(_boost_PATH_SUFFIXES)
    foreach(_boost_VER ${_boost_TEST_VERSIONS})
        # Add in a path suffix, based on the required version, ideally we could read this from version.hpp, but for that
        # to work we'd need to know the include dir already
        ...
        list(APPEND _boost_PATH_SUFFIXES "boost-${_boost_BOOSTIFIED_VERSION}" "boost_${_boost_BOOSTIFIED_VERSION}"
...
if(Boost_INCLUDE_DIR)
    _boost_debug_print("${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}"
                       "location of version.hpp: ${Boost_INCLUDE_DIR}/hboost/version.hpp")

    # Extract Boost_VERSION_MACRO and Boost_LIB_VERSION from version.hpp
    set(Boost_VERSION_MACRO 0)
    set(Boost_LIB_VERSION "")
    file(STRINGS "${Boost_INCLUDE_DIR}/hboost/version.hpp" _boost_VERSION_HPP_CONTENTS
         REGEX "#define HBOOST_(LIB_)?VERSION ")
    if("${_boost_VERSION_HPP_CONTENTS}" MATCHES "#define HBOOST_VERSION ([0-9]+)")
        set(Boost_VERSION_MACRO "${CMAKE_MATCH_1}")
    endif()
    if("${_boost_VERSION_HPP_CONTENTS}" MATCHES "#define HBOOST_LIB_VERSION \"([0-9_]+)\"")
    ...
    cmake_policy(GET CMP0093 _Boost_CMP0093 PARENT_SCOPE # undocumented, do not use outside of CMake
    )
...
if(NOT DEFINED Boost_LIB_PREFIX)
    # Boost's static libraries use a "lib" prefix on DLL platforms to distinguish them from the DLL import libraries.
...
if(Boost_COMPILER)
    ...
    _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "_boost_COMPILER" SOURCE
                           "user-specified via Boost_COMPILER")
else()
    # Attempt to guess the compiler suffix NOTE: this is not perfect yet, if you experience any issues please report
    # them and use the Boost_COMPILER variable to work around the problems.
...
# http://boost.org/doc/libs/1_66_0/tools/build/src/tools/common.jam http://boost.org/doc/libs/1_66_0/boostcpp.jam
...
# n        using the STLport deprecated "native iostreams" feature removed from the documentation in 1.43.0 but still
# present in boost/config/auto_link.hpp
if(Boost_USE_STLPORT_DEPRECATED_NATIVE_IOSTREAMS)
    string(APPEND _boost_RELEASE_ABI_TAG "n")
    string(APPEND _boost_DEBUG_ABI_TAG "n")
endif()

# -x86     Architecture and address model tag First character is the architecture, then word-size, either 32 or 64 Only
# used in 'versioned' layout, added in Boost 1.66.0
if(DEFINED Boost_ARCHITECTURE)
    set(_boost_ARCHITECTURE_TAG "${Boost_ARCHITECTURE}")
    _boost_debug_print_var("${CMAKE_CURRENT_LIST_FILE}" "${CMAKE_CURRENT_LIST_LINE}" "_boost_ARCHITECTURE_TAG" SOURCE
                           "user-specified via Boost_ARCHITECTURE")
else()
    ...
    if(NOT "x${CMAKE_CXX_COMPILER_ARCHITECTURE_ID}" STREQUAL "x" AND NOT Boost_VERSION_STRING VERSION_LESS 1.66.0)
        ...
        # This needs to be kept in-sync with the section of CMakePlatformId.h.in inside 'defined(_WIN32) &&
        # defined(_MSC_VER)'
        if(CMAKE_CXX_COMPILER_ARCHITECTURE_ID STREQUAL "IA64")
            string(APPEND _boost_ARCHITECTURE_TAG "i")
        elseif(CMAKE_CXX_COMPILER_ARCHITECTURE_ID STREQUAL "X86" OR CMAKE_CXX_COMPILER_ARCHITECTURE_ID STREQUAL "x64")
...
foreach(c DEBUG RELEASE)
    ...
    if(Boost_LIBRARY_DIR_${c})
        ...
        list(APPEND _boost_LIBRARY_SEARCH_DIRS_${c} ${Boost_INCLUDE_DIR}/lib ${Boost_INCLUDE_DIR}/../lib
...
if(Boost_FOUND)
    if(NOT Boost_LIBRARY_DIRS)
        ...
        # Look for the boost library path. Note that the user may not have installed any libraries so it is quite
        # possible the Boost_LIBRARY_DIRS may not exist.
...
if(Boost_FOUND)
    # The builtin CMake package in Boost 1.70+ introduces a new name for the header-only lib, let's provide the same UI
    # in module mode
    if(NOT TARGET Boost::headers)
        add_library(Boost::headers INTERFACE IMPORTED)
    ...
    foreach(COMPONENT ${Boost_FIND_COMPONENTS})
        if(_Boost_IMPORTED_TARGETS AND NOT TARGET Boost::${COMPONENT})
            ...
            if(Boost_${UPPERCOMPONENT}_FOUND)
                ...
                if(Boost_INCLUDE_DIRS)
                    set_target_properties(Boost::${COMPONENT} PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                                                         "${Boost_INCLUDE_DIRS}")
                endif()
    ...
    if(WIN32)
        # In windows, automatic linking is performed, so you do not have to specify the libraries.  If you are linking
        # to a dynamic runtime, then you can choose to link to either a static or a dynamic Boost library, the default
        # is to do a static link.  You can alter this for a specific library "whatever" by defining
        # BOOST_WHATEVER_DYN_LINK to force Boost library "whatever" to be linked dynamically.  Alternatively you can
        # force all Boost libraries to dynamic link by defining BOOST_ALL_DYN_LINK.

        # This feature can be disabled for Boost library "whatever" by defining BOOST_WHATEVER_NO_LIB, or for all of
        # Boost by defining BOOST_ALL_NO_LIB.

        # If you want to observe which libraries are being linked against then defining BOOST_LIB_DIAGNOSTIC will cause
        # the auto-linking code to emit a #pragma message each time a library is selected for linking.
